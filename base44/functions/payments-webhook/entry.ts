...
async function handleOrderApproved(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Grant the buyer's plan. The User entity has a writable `plan` field; an update is idempotent.
  // For a signed-in buyer, update by appUserId; for an anonymous buyer, match buyerEmail to a User.
...
async function handleSubscriptionEnded(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Revoke the plan: set the user back to the free plan. Idempotent.
