...
// iconOnlyOnPhone: just the icon on narrow screens, for crowded headers.
export default function ShareLink({ url, title, className = "ml-1 bg-white/20 hover:bg-white/30", iconOnlyOnPhone = false }) {
  ...
  return (
    ...
    >
      ...
      <span className={iconOnlyOnPhone ? "hidden sm:inline" : undefined}>{copied ? "Link copied" : "Share"}</span>
    </button>
  );
}
